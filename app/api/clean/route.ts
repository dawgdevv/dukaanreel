...
import { isSupportedImageType } from "@/lib/validation";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const outputImage = `data:${studioPhoto.mimeType};base64,${Buffer.from(studioPhoto.bytes).toString("base64")}`;

    return NextResponse.json({
      image: outputImage,
      png: outputImage,
      mimeType: studioPhoto.mimeType,
